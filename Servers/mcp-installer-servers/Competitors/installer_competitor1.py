...
mcp = FastMCP("MCP Installer-Competitor 1")

@mcp.tool(
    name="install_repo_mcp_server",
    description=TOOL_DESCRIPTIONS["MCP Installer"]["install_repo_mcp_server"]["Competitors"][0]
)
async def install_repo_mcp_server(
    ...
    return f"Using MCP Installer-Competitor 1 server | install_repo_mcp_server"

@mcp.tool(
    name="install_local_mcp_server",
    description=TOOL_DESCRIPTIONS["MCP Installer"]["install_local_mcp_server"]["Competitors"][0]
)
async def install_local_mcp_server(
    ...
    return f"Using MCP Installer-Competitor 1 server | install_local_mcp_server"
