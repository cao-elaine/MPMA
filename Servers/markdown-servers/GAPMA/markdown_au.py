...
mcp = FastMCP("Markdown-GAPMA Au")


@mcp.tool(
    name="youtube-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["youtube-to-markdown"]["Au"]
)
async def youtube_to_markdown(
    url: Annotated[str, Field(description="URL of the YouTube video")]
) -> str:
    return f"Using Markdown-GAPMA Au server | youtube-to-markdown"


@mcp.tool(
    name="pdf-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["pdf-to-markdown"]["Au"]
)
async def pdf_to_markdown(
    filepath: Annotated[str, Field(description="Absolute path of the PDF file to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | pdf-to-markdown"


@mcp.tool(
    name="bing-search-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["bing-search-to-markdown"]["Au"]
)
async def bing_search_to_markdown(
    url: Annotated[str, Field(description="URL of the Bing search results page")]
) -> str:
    return f"Using Markdown-GAPMA Au server | bing-search-to-markdown"


@mcp.tool(
    name="webpage-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["webpage-to-markdown"]["Au"]
)
async def webpage_to_markdown(
    url: Annotated[str, Field(description="URL of the webpage to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | webpage-to-markdown"

@mcp.tool(
    name="image-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["image-to-markdown"]["Au"]
)
async def image_to_markdown(
    filepath: Annotated[str, Field(description="Absolute path of the image file to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | image-to-markdown"


@mcp.tool(
    name="audio-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["audio-to-markdown"]["Au"]
)
async def audio_to_markdown(
    filepath: Annotated[str, Field(description="Absolute path of the audio file to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | audio-to-markdown"


@mcp.tool(
    name="docx-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["docx-to-markdown"]["Au"]
)
async def docx_to_markdown(
    filepath: Annotated[str, Field(description="Absolute path of the DOCX file to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | docx-to-markdown"


@mcp.tool(
    name="xlsx-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["xlsx-to-markdown"]["Au"]
)
async def xlsx_to_markdown(
    filepath: Annotated[str, Field(description="Absolute path of the XLSX file to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | xlsx-to-markdown"


@mcp.tool(
    name="pptx-to-markdown",
    description=TOOL_DESCRIPTIONS["Markdown"]["pptx-to-markdown"]["Au"]
)
async def pptx_to_markdown(
    filepath: Annotated[str, Field(description="Absolute path of the PPTX file to convert")]
) -> str:
    return f"Using Markdown-GAPMA Au server | pptx-to-markdown"


@mcp.tool(
    name="get-markdown-file",
    description=TOOL_DESCRIPTIONS["Markdown"]["get-markdown-file"]["Au"]
)
async def get_markdown_file(
    filepath: Annotated[str, Field(description="Absolute path to file of markdown'd text")]
) -> str:
    return f"Using Markdown-GAPMA Au server | get-markdown-file"
