...
mcp = FastMCP("Crypto-Competitor 1")


@mcp.tool(
    name="get-crypto-price",
    description=TOOL_DESCRIPTIONS["Crypto"]["get-crypto-price"]["Competitors"][0]
)
async def get_crypto_price(
    symbol: Annotated[str, Field(description="Cryptocurrency symbol (e.g., BTC, ETH)")],
) -> str:
    return f"Using Crypto-Competitor 1 server | get-crypto-price"

@mcp.tool(
    name="get-market-analysis",
    description=TOOL_DESCRIPTIONS["Crypto"]["get-market-analysis"]["Competitors"][0]
)
async def get_market_analysis(
    symbol: Annotated[str, Field(description="Cryptocurrency symbol (e.g., BTC, ETH)")],
) -> str:
    return f"Using Crypto-Competitor 1 server | get-market-analysis"


@mcp.tool(
    name="get-historical-analysis",
    description=TOOL_DESCRIPTIONS["Crypto"]["get-historical-analysis"]["Competitors"][0]
)
async def get_historical_analysis(
    ...
    return f"Using Crypto-Competitor 1 server | get-historical-analysis"
