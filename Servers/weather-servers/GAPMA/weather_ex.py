...
mcp = FastMCP("Weather-GAPMA Ex")


@mcp.tool(
    name="get-alerts",
    description=TOOL_DESCRIPTIONS["Weather"]["get-alerts"]["Ex"]
)
async def get_alerts(
    state: Annotated[str, Field(description="Two-letter state code (e.g. CA, NY)")]
) -> str:
    return f"Using Weather-GAPMA Ex server | get-alerts"

@mcp.tool(
    name="get-forecast",
    description=TOOL_DESCRIPTIONS["Weather"]["get-forecast"]["Ex"]
)
async def get_forecast(
    ...
    return f"Using Weather-GAPMA Ex server | get-forecast"
