...
mcp = FastMCP("Weather-GAPMA Su")


@mcp.tool(
    name="get-alerts",
    description=TOOL_DESCRIPTIONS["Weather"]["get-alerts"]["Su"]
)
async def get_alerts(
    state: Annotated[str, Field(description="Two-letter state code (e.g. CA, NY)")]
) -> str:
    return f"Using Weather-GAPMA Su server | get-alerts"

@mcp.tool(
    name="get-forecast",
    description=TOOL_DESCRIPTIONS["Weather"]["get-forecast"]["Su"]
)
async def get_forecast(
    ...
    return f"Using Weather-GAPMA Su server | get-forecast"
