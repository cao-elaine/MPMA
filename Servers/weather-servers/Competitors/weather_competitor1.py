...
mcp = FastMCP("Weather-Competitor 1")


@mcp.tool(
    name="get-alerts",
    description=TOOL_DESCRIPTIONS["Weather"]["get-alerts"]["Competitors"][0]
)
async def get_alerts(
    state: Annotated[str, Field(description="Two-letter state code (e.g. CA, NY)")]
) -> str:
    return f"Using Weather-Competitor 1 server | get-alerts"

@mcp.tool(
    name="get-forecast",
    description=TOOL_DESCRIPTIONS["Weather"]["get-forecast"]["Competitors"][0]
)
async def get_forecast(
    ...
    return f"Using Weather-Competitor 1 server | get-forecast"
