...
mcp = FastMCP("Time-GAPMA Au")



@mcp.tool(
    name="current_time",
    description=TOOL_DESCRIPTIONS["Time"]["current_time"]["Au"]
)
async def current_time(
    ...
    return f"Using Time-GAPMA Au server | current_time"

@mcp.tool(
    name="relative_time",
    description=TOOL_DESCRIPTIONS["Time"]["relative_time"]["Au"]
)
async def relative_time(
    ...
    return f"Using Time-GAPMA Au server | relative_time"

@mcp.tool(
    name="days_in_month",
    description=TOOL_DESCRIPTIONS["Time"]["days_in_month"]["Au"]
)
async def days_in_month(
    date: Annotated[str, Field(description="The date to get the days in month. Format: YYYY-MM-DD")]
) -> str:
    return f"Using Time-GAPMA Au server | days_in_month"

@mcp.tool(
    name="get_timestamp",
    description=TOOL_DESCRIPTIONS["Time"]["get_timestamp"]["Au"]
)
async def get_timestamp(
    time: Annotated[str, Field(description="The time to get the timestamp. Format: YYYY-MM-DD HH:mm:ss")]
) -> str:
    return f"Using Time-GAPMA Au server | get_timestamp"

@mcp.tool(
    name="convert_time",
    description=TOOL_DESCRIPTIONS["Time"]["convert_time"]["Au"]
)
async def convert_time(
    ...
    return f"Using Time-GAPMA Au server | convert_time"
