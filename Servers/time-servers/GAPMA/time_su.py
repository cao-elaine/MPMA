...
mcp = FastMCP("Time-GAPMA Su")


@mcp.tool(
    name="current_time",
    description=TOOL_DESCRIPTIONS["Time"]["current_time"]["Su"]
)
async def current_time(
    ...
    return f"Using Time-GAPMA Su server | current_time"

@mcp.tool(
    name="relative_time",
    description=TOOL_DESCRIPTIONS["Time"]["relative_time"]["Su"]
)
async def relative_time(
    ...
    return f"Using Time-GAPMA Su server | relative_time"

@mcp.tool(
    name="days_in_month",
    description=TOOL_DESCRIPTIONS["Time"]["days_in_month"]["Su"]
)
async def days_in_month(
    date: Annotated[str, Field(description="The date to get the days in month. Format: YYYY-MM-DD")]
) -> str:
    return f"Using Time-GAPMA Su server | days_in_month"

@mcp.tool(
    name="get_timestamp",
    description=TOOL_DESCRIPTIONS["Time"]["get_timestamp"]["Su"]
)
async def get_timestamp(
    time: Annotated[str, Field(description="The time to get the timestamp. Format: YYYY-MM-DD HH:mm:ss")]
) -> str:
    return f"Using Time-GAPMA Su server | get_timestamp"

@mcp.tool(
    name="convert_time",
    description=TOOL_DESCRIPTIONS["Time"]["convert_time"]["Su"]
)
async def convert_time(
    ...
    return f"Using Time-GAPMA Su server | convert_time"
