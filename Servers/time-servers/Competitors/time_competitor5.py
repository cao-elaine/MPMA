...
mcp = FastMCP("Time-Competitor 5")


@mcp.tool(
    name="current_time",
    description=TOOL_DESCRIPTIONS["Time"]["current_time"]["Competitors"][4]
)
async def current_time(
    ...
    return f"Using Time-Competitor 5 server | current_time"

@mcp.tool(
    name="relative_time",
    description=TOOL_DESCRIPTIONS["Time"]["relative_time"]["Competitors"][4]
)
async def relative_time(
    ...
    return f"Using Time-Competitor 5 server | relative_time"

@mcp.tool(
    name="days_in_month",
    description=TOOL_DESCRIPTIONS["Time"]["days_in_month"]["Competitors"][4]
)
async def days_in_month(
    date: Annotated[str, Field(description="The date to get the days in month. Format: YYYY-MM-DD")]
) -> str:
    return f"Using Time-Competitor 5 server | days_in_month"

@mcp.tool(
    name="get_timestamp",
    description=TOOL_DESCRIPTIONS["Time"]["get_timestamp"]["Competitors"][4]
)
async def get_timestamp(
    time: Annotated[str, Field(description="The time to get the timestamp. Format: YYYY-MM-DD HH:mm:ss")]
) -> str:
    return f"Using Time-Competitor 5 server | get_timestamp"

@mcp.tool(
    name="convert_time",
    description=TOOL_DESCRIPTIONS["Time"]["convert_time"]["Competitors"][4]
)
async def convert_time(
    ...
    return f"Using Time-Competitor 5 server | convert_time"
